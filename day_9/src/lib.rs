...
pub fn part_two(input: &str) -> i32 {
    ...
    for history in histories {
        ...
        let mut increase_by = 0;
        for mut sequence in sequences {
            let new_sequence_number = sequence.first().unwrap() - increase_by;
            sequence.insert(0, new_sequence_number);
...
mod tests {
    ...
    #[test]
    fn example2() {
        assert_eq!(super::part_two(include_str!("example.txt")), 2);
    }

    #[test]
    fn part_two() {
        assert_eq!(super::part_two(include_str!("input.txt")), 957);
    }
}
